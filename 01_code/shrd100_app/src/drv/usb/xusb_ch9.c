			...
			SetupPacket *SetupData)
{
...
#ifdef CH9_DEBUG
	printf("Handle setup packet, RequestType:%x\n", SetupData->bRequestType);
#endif
	...
	case USB_CMD_CLASSREQ:
		#ifdef CH9_DEBUG
			printf("***class req packet,request:%x\n", SetupData->bRequest);
#endif
	...
	default:
		/* Stall on Endpoint 0 */
#ifdef CH9_DEBUG
		xil_printf("unknown class req, stalling at %s\n", __func__);
#endif
...
#pragma data_alignment = 32
	...
	case USB_REQ_GET_STATUS:
		...
		case USB_STATUS_DEVICE:
			 ...
			 */
			 #if 1 ///CONFIG_USB_SELF_POWRED //FIXME
			*((u16 *) &Reply[0]) = 0x0100; /* Self powered */
			 #else
			 *((u16 *) &Reply[0]) = 0; /* powered by host */
			 #endif
