...
static void req_handler(void *core_mailbox)
{
	...
	if (MAILBOX_MSG_TYPE_REQ == req_msg->type)
	{
		if (NULL == mailbox->recv_cb)
		{
			...
			xil_printf("%s[%d]no call back function, send NACK\r\n", __func__, __LINE__);
			ret = -1;
		}
		else
		{
...
int core_mailbox_send(core_mailbox_t *core_mailbox, const void *data, uint32_t size, unsigned int time_out_ms)
{
	...
	while(1)
	{
		//xil_printf("%s[%d]wait for function call signal\r\n", __func__, __LINE__);
		...
		//xil_printf("%s[%d]got call signal, ret:%d\r\n", __func__, __LINE__, wait_ret);
		if (pdTRUE == wait_ret)
		{
...
void mailbox_demo(void)
{
	...
	ret = core_mailbox_init(&writer, TARGET_CORE_ID, 0, 0, CORE_MAILBOX_TYPE_WRITE);
	xil_printf("writer init:%d\r\n", ret);

	ret = core_mailbox_init(&reader, TARGET_CORE_ID, 1, 1, CORE_MAILBOX_TYPE_READ);
	xil_printf("reader init:%d\r\n", ret);

	core_mailbox_set_recv_cb(&reader, reader_cb, reader_buffer);

	///vTaskDelay(200); //wait core1 ready

	while(1)
	{
		memset(buffer, 0, sizeof(buffer));
		sprintf(buffer, "[%d]app0 writer", cnt);

		ret = core_mailbox_send(&writer, buffer, strlen((const char *)buffer), 1000);
		xil_printf("[%d]1send:%d\r\n", cnt, ret);
